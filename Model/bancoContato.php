...
function buscarContatoId($conexao, $idContato){
    $query = "Select * contatotb where idContato like '%{idContato}%'";
    $result = mysqli_query($conexao, $query);
    return $result;
...
function alterarContato($conexao, $idContato, $nomeContato, $foneContato){
    $query = "update contatotb set nomeContato = '{$nomeContato}', foneContato = '{$foneContato}' where idContato = '{$idContato}'";
    $result = mysqli_query($conexao, $query);
    return $result;
